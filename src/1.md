                    ...
                    <div className="swap-header">
                      ...
                      <div className="settings-row">
                        <div className="aggregator-toggle" ref={switchRef}>
                          <label className="chakra-form__label gradient-text" htmlFor="aggregator-mode">Mine $SuiFlow tokens</label>
...
function LaunchPage() {
  ...
  const handleReadDoc = () => {
    window.open('https://SuiFlowprotocol.gitbook.io', '_blank');
  };
...
18.2秒
已中断
你应该给我新的页面的tsx和css文件，还有launchpage修改后的tsx文件（因为faucet for test按钮要链接到新页面）：目前的页面launchPage有一个Faucet for test按钮，点击后帮我新建一个页面，用户可以分别领取DEEP WAL USDC IKA BLUE SuiFlow测试币。包括显示水龙头余量（所有代币均为6位小数）。和向水龙头注入每种代币的功能。保持页面美观。链上move合约的信息：package id：0x16c59472305767de5d4ff834240c46ee537ab6be4503477abfaaf33ecb26e668 。合约源码：#[allow(duplicate_alias)]
module faucet::faucet {
    use sui::coin;
    use sui::balance;
    use sui::clock;
